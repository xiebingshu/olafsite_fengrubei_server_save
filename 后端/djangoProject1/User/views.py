...
class Register:
    def __init__(self, username, password, emailbox):
    ...
    def register(self):
        try:
            user=UserInfo.objects.get(name=self.username)
            self.result = False
        except Exception as e:
            UserInfo.objects.create(
                name=self.username,
                password=self.password,
                emailbox=self.emailbox
            )
...
class Login:
    ...
    def login(self):
        try:
            user=UserInfo.objects.get(Q(name=self.username)&Q(password=self.password))
            self.result = True
        except Exception as e:
            self.result = False
...
@csrf_exempt
def index(request):
    Res = Register(request.POST.get("username"), request.POST.get("pass"), request.POST.get("email"))
    Res.register()
...
def captcha(request):
    ...
    verification = SendEmail(data=data, receiver=receiver_email).send_email()
...
def login(request):
    print(request)
    Log = Login(request.POST.get("username"), request.POST.get("pass"))
    Log.login()
    return HttpResponse(Log.result)